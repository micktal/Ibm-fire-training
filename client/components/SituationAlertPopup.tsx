  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: 0 }}>

          {/* LEFT — Message body */}
          <div className="px-5 py-4" style={{ borderRight: "1px solid rgba(255,255,255,0.08)" }}>
            {/* Sender row */}
            <div className="flex items-center gap-3 mb-3">
              <div
                className="w-9 h-9 rounded-full flex items-center justify-center font-bold flex-shrink-0"
                style={{ background: alert.senderColor, color: "#fff", fontSize: "12px", letterSpacing: "0.03em" }}
              >
                {alert.senderInitials}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-white" style={{ fontSize: "0.875rem" }}>{alert.senderName}</div>
            ...
            {/* Subject (email only) */}
            {isEmail && (
              <div className="font-bold mb-2" style={{ color: "#fff", fontSize: "0.9375rem", lineHeight: "1.3" }}>
                {alertSubject}
              </div>
            )}
            ...
            <div className="rounded-xl p-3.5" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              {alertBody.split("\n\n").map((para, i) => (
                <p key={i} style={{ color: "rgba(255,255,255,0.8)", fontSize: "0.84rem", lineHeight: "1.6", marginBottom: i < alertBody.split("\n\n").length - 1 ? "0.6rem" : 0 }}>
                  {para}
                </p>
              ))}
            </div>
          ...
          {/* RIGHT — Question + choices + feedback + button */}
          <div className="px-5 py-4 flex flex-col gap-3">
            {/* Question box */}
            <div className="rounded-xl px-4 py-3" style={{ background: urgency.bg, border: `1.5px solid ${urgency.border}` }}>
              ...
              <div className="font-bold text-white" style={{ fontSize: "0.875rem", lineHeight: "1.4" }}>
                {alertQuestion}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              {alert.choices.map((choice) => {
                ...
                if (showResult && isCorrect) { bg = "rgba(25,128,56,0.18)"; border = "rgba(25,128,56,0.5)"; textColor = "#6fdc8c"; }
                else if (showResult && isSelected && !isCorrect) { bg = "rgba(218,30,40,0.18)"; border = "rgba(218,30,40,0.5)"; textColor = "#ff8b8b"; }
                ...
                return (
                  <button
                    ...
                    disabled={answered}
                    className="w-full flex items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-left transition-all"
                    style={{ background: bg, border: `1.5px solid ${border}`, cursor: answered ? "default" : "pointer" }}
                    onMouseEnter={(e) => { if (!answered) (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.08)"; }}
                    onMouseLeave={(e) => { if (!answered) (e.currentTarget as HTMLElement).style.background = bg; }}
                  >
                    <span className="font-mono font-bold flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center"
                      style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: "11px", background: showResult && isCorrect ? "#198038" : showResult && isSelected ? "#da1e28" : "rgba(255,255,255,0.1)", color: "#fff" }}
                    >{choice.key}</span>
                    <span style={{ color: textColor, fontSize: "0.83rem", lineHeight: "1.4", flex: 1 }}>{choiceLabel(choice)}</span>
                    {showResult && isCorrect && <CheckCircle2 size={14} color="#6fdc8c" className="flex-shrink-0" />}
                    {showResult && isSelected && !isCorrect && <XCircle size={14} color="#ff8b8b" className="flex-shrink-0" />}
                  </button>
                );
              })}
            </div>
            ...
            {answered && selectedChoice && (
              <div className="rounded-xl px-3.5 py-2.5" style={{ background: selectedChoice.correct ? "rgba(25,128,56,0.12)" : "rgba(218,30,40,0.1)", border: `1.5px solid ${selectedChoice.correct ? "rgba(25,128,56,0.35)" : "rgba(218,30,40,0.35)"}` }}>
                <div className="flex items-start gap-2">
                  {selectedChoice.correct ? <CheckCircle2 size={13} color="#6fdc8c" className="flex-shrink-0 mt-0.5" /> : <XCircle size={13} color="#ff8b8b" className="flex-shrink-0 mt-0.5" />}
                  <div>
                    <div className="font-bold text-xs mb-0.5" style={{ color: selectedChoice.correct ? "#6fdc8c" : "#ff8b8b" }}>
                      {selectedChoice.correct
                        ? (isEN ? "Good reaction" : "Bonne réaction")
                        : (isEN ? `Incorrect — Correct: ${correctChoice ? choiceLabel(correctChoice) : ""}` : `Incorrect — La bonne : ${correctChoice ? choiceLabel(correctChoice) : ""}`)}
                    </div>
                    <div style={{ color: "rgba(255,255,255,0.65)", fontSize: "0.8rem", lineHeight: "1.5" }}>
            ...
            {/* Continue button — always at bottom of right panel */}
            <div className="mt-auto">
              {answered && (
                <button
                  onClick={onClose}
                  className="w-full flex items-center justify-center gap-2.5 rounded-2xl py-3 font-bold transition-all"
                  style={{ background: "linear-gradient(135deg, #0043ce 0%, #0f62fe 100%)", color: "#fff", border: "none", cursor: "pointer", fontSize: "0.9375rem", boxShadow: "0 8px 32px rgba(15,98,254,0.4)" }}
                >
