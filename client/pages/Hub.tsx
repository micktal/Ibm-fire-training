...
import { Lock, CheckCircle2, Clock, Play, Flame, Shield, Award, ChevronLeft, ChevronRight, Home, Menu, BarChart2 } from "lucide-react";
import IBMLogo from "@/components/IBMLogo";
import { useUser } from "@/lib/userContext";
import { getChapterModules, CourseModule } from "@/lib/courseData";

// ── Geometric background (same as Index) ────────────────────────
...
export default function Hub() {
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      >
        <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors" style={{ color: "#0D47A1" }} onClick={() => navigate(-1)}>
          <ChevronLeft size={18} />
        </button>
